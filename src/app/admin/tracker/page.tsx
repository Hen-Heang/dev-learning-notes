...
const COLUMNS: { id: StudyTaskStatus; label: string; icon: React.ElementType; color: string; bg: string; border: string }[] = [
  { id: "todo",  label: "To Do",      icon: Circle,       color: "text-zinc-400",   bg: "bg-zinc-800/40",    border: "border-zinc-700/50" },
  { id: "doing", label: "In Progress",icon: Clock,        color: "text-amber-400",  bg: "bg-amber-400/5",    border: "border-amber-400/20" },
  { id: "done",  label: "Done",       icon: CheckCircle2, color: "text-emerald-400",bg: "bg-emerald-400/5",  border: "border-emerald-400/20" },
];
  ...
  return (
    <div className="px-4 sm:px-6 py-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          <Link href="/admin" className="flex items-center gap-1 text-xs font-mono text-zinc-600 hover:text-zinc-400 transition-colors mb-2">
            <ChevronLeft size={13} /> owner panel
          </Link>
          <h1 className="text-2xl font-bold text-zinc-100 flex items-center gap-2.5">
            <ListChecks size={22} className="text-amber-400" />
            Study Tracker
          </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          { label: "Progress", value: `${stats.pct}%`,    tone: "text-amber-300" },
          { label: "Done",     value: `${stats.done}`,    tone: "text-emerald-300" },
          { label: "Doing",    value: `${stats.doing}`,   tone: "text-sky-300" },
          { label: "Todo",     value: `${stats.todo}`,    tone: "text-zinc-300" },
        ].map(c => (
          <div key={c.label} className="rounded-xl border border-zinc-800 bg-zinc-900/50 px-4 py-3">
            <p className="text-[10px] font-mono uppercase tracking-widest text-zinc-600">{c.label}</p>
      ...
      {/* Progress bar */}
      <div className="h-1.5 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800 mb-8">
        <motion.div
      ...
      <AnimatePresence>
        {notice && (
          <motion.p initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="text-sm text-emerald-300 bg-emerald-400/10 border border-emerald-400/20 rounded-lg px-4 py-3 mb-5">
            {notice}
          </motion.p>
        )}
        {error && (
          <motion.p initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="text-sm text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-4 py-3 mb-5">
            {error}
          </motion.p>
        )}
      </AnimatePresence>
            ...
            return (
              <div key={col.id} className={`rounded-2xl border ${col.border} ${col.bg} p-4 min-h-48`}>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs font-mono text-zinc-600 tabular-nums">{colTasks.length}</span>
                </div>
                ...
                <div className="space-y-2.5">
                  <AnimatePresence mode="popLayout">
                    {colTasks.map(task => (
                      <motion.div
                        ...
                        className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 group"
                      >
                        {/* Card top row */}
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex flex-wrap gap-1.5">
                            <span className="text-[10px] font-mono text-amber-400/80 bg-amber-400/8 border border-amber-400/15 px-1.5 py-0.5 rounded-full">
                              {task.phase}
                            </span>
                            <span className="text-[10px] font-mono text-zinc-500 bg-zinc-800 border border-zinc-700 px-1.5 py-0.5 rounded-full">
                              {task.category}
                            </span>
                          </div>
                          {!task.id.startsWith("local-") && (
                            <button
                              ...
                              className="opacity-0 group-hover:opacity-100 p-1 text-zinc-600 hover:text-red-400 hover:bg-red-400/10 rounded-md transition-all"
                            >
                              <Trash2 size={12} />
                            </button>
                          )}
                        ...
                        <p className="text-sm font-medium text-zinc-100 leading-snug mb-2">{task.title}</p>
                        {task.notes && (
                          <p className="text-xs text-zinc-600 leading-relaxed mb-3">{task.notes}</p>
                        )}
                  ...
                  {colTasks.length === 0 && (
                    <p className="text-center text-xs text-zinc-700 py-6 font-mono">empty</p>
                  )}
                </div>
              </div>
            );
        ...
        {showForm && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-zinc-950/80 backdrop-blur-sm z-40" onClick={() => setShowForm(false)} />
            <motion.div
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
                <h2 className="text-sm font-semibold text-zinc-100">New Study Task</h2>
                <button onClick={() => setShowForm(false)} className="p-1.5 text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 rounded-lg transition-colors">
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input
                      ...
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-amber-400 transition-colors"
                    />
                  </div>
                ))}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-amber-400 transition-colors"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-amber-400 transition-colors resize-none"
                  />
                </div>
              </div>
